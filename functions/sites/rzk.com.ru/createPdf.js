...
function generateHeader(doc, data) {
  ...
  // obj info
  for (const phoneNumber of data.object.phoneArray) {
  ...
  doc.font(fontRegular).fontSize(10).fillColor("blue").text(`${data.domain}/o/${data.object.id}`, columnPosition + 250, rowPosition, {align: "right", link: `${data.domain}/o/${data.object.id}`});
  rowPosition += 15;
}
...
function generateCustomerInformation(doc, data) {
  ...
  if (data.type === "cart") {
    doc.font(fontRegular).fontSize(14).fillColor("#000").text(`${data.object.name} / ${data.i18n.cart}`, columnPosition, rowPosition, {align: "center"});
...
function generateFooter(doc, data) {
  if (data.type === "cart") {
    ...
    doc.fillColor("blue")
        .text(`${data.domain}/o/${data.object.id}/cart?${products}`,
            columnPosition, rowPosition + 15, {width: 550, link: `${data.domain}/o/${data.object.id}/cart?${products}`});
  } else {
    doc.fillColor("blue").fontSize(10)
        .text(`${data.domain}/o/${data.object.id}/s/${data.order.id}`,
            columnPosition, rowPosition + 15, {width: 550, link: `${data.domain}/o/${data.object.id}/s/${data.order.id}`});
  }
  // encode uri
  // doc.fillColor("blue")
